...
public class TodosAdapter extends RecyclerView.Adapter<TodosAdapter.MyViewHolder> {
    ...
    public TodosAdapter(ArrayList<Todo> p, Context c) {
        todos = p;
        context = c;
    }
    ...
    public void onBindViewHolder(@NonNull final MyViewHolder myViewHolder, int i) {
        ...
        myViewHolder.itemTodoBtnDone.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                todoRepository = new TodoRepository(v.getContext());
                todoRepository.updateTodo(idTodo, titleTodo, deadlineTodo, "done");
                Toast.makeText(v.getContext(), todos.get(i).getTitle() + " Done", Toast.LENGTH_SHORT).show();
                removeAt(i);
            }
        });
